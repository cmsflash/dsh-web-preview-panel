...
export function apply(ctx, config) {
  // 链接打开策略（由 profile 的 cordis.patch.yml 提供；缺省值在安装清单里声明）：
  // externalLinks 'browser' 默认放行外网链接到系统浏览器，'panel' 默认在侧边预览打开；
  // panelHosts / browserHosts 按域名强制覆盖默认（精确匹配或 dot 子域后缀）。
  const cfg = {
  ...
  if (cfg.externalLinks !== 'browser' && cfg.externalLinks !== 'panel') {
    throw new Error('dsh-web-preview-panel: externalLinks 必须是 "browser" 或 "panel"，实际为 ' + JSON.stringify(cfg.externalLinks))
  }
  if (!Array.isArray(cfg.panelHosts) || !Array.isArray(cfg.browserHosts)) {
    throw new Error('dsh-web-preview-panel: panelHosts / browserHosts 必须是字符串数组')
  }
  ...
  api['get-config'] = async () => ({
    ...
    policy: {
      ...
      panelHosts: cfg.panelHosts.slice(),
      browserHosts: cfg.browserHosts.slice()
    }
  })
