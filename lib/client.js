...
function apply(ctx) {
  ...
  // ---------- 链接打开策略 ----------
  // 默认放行外网链接到系统浏览器、本地路径留在侧边预览；宿主半经 get-config
  // 下发 profile 配置（externalLinks / panelHosts / browserHosts）。
  // get-config 不可用（宿主半早于该接口）时保持内置默认。
  let linkPolicy = { externalLinks: 'browser', panelHosts: [], browserHosts: [] }
  call('get-config', {}).then((res) => {
    if (res && res.policy && typeof res.policy === 'object') linkPolicy = res.policy
  }).catch(() => { /* noop */ })

  const hostMatches = (list, host) => list.some((entry) => {
    const d = String(entry || '').toLowerCase().replace(/^\*\./, '')
    return host === d || host.endsWith('.' + d)
  })

  const releasesToBrowser = (href) => {
    try {
      ...
      if (hostMatches(linkPolicy.panelHosts || [], host)) return false
      if (hostMatches(linkPolicy.browserHosts || [], host)) return true
      return linkPolicy.externalLinks !== 'panel'
    } catch (e) {
      return false
    }
  }
  ...
  const convertTextNode = (textNode) => {
    ...
    while ((m = TEXTLINK_RE.exec(text))) {
      ...
      a.title = /^https?:\/\//i.test(m[0]) ? '打开链接' : '在侧边预览中打开 '
      a.textContent = m[0]
      frag.appendChild(a)
  ...
  try {
    ...
    linkObserver = new MutationObserver((muts) => {
      ...
      // 流式渲染的后续 commit 会销毁刚注入的链接；DOM 安静 1.5 秒后对会话区
      // 做一次幂等补扫，让收尾状态的纯文本路径重新变成链接。
      if (settleTimer) clearTimeout(settleTimer)
      settleTimer = setTimeout(() => {
        settleTimer = null
        document.querySelectorAll('[data-slot="conversation.session"]').forEach((scope) => {
          try { processNode(scope) } catch (e) { /* noop */ }
        })
      }, 1500)
    })
